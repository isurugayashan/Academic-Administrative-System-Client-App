
import {Substatus} from "../entity/substatus";
import {Civilstatus} from "../entity/civilstatus";
import {Qualification} from "../entity/qualifications";
import {Gradetype} from "../entity/gradetype";
import {Grade} from "../entity/grade";
import {Empstatus} from "../entity/empstatus";
import {Officetype} from "../entity/officetype";
import {Injectable} from "@angular/core";
import {HttpClient} from "@angular/common/http";

@Injectable({
  providedIn: 'root'
})

export class Substatusservice{

  constructor( private http: HttpClient) {}

  // @ts-ignore
 async getAllList(): Promise<Array<Substatus>>{

    const substatuses = await this.http.get<Array<Substatus>>('http://localhost:8080/substatuses/list').toPromise();
    if (substatuses == undefined){
      return [];
    }

    return substatuses;
 }
}
