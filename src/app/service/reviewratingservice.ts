
import {Reviewrating} from "../entity/reviewrating";
import {Civilstatus} from "../entity/civilstatus";
import {Qualification} from "../entity/qualifications";
import {Gradetype} from "../entity/gradetype";
import {Grade} from "../entity/grade";
import {Empstatus} from "../entity/empstatus";
import {Officetype} from "../entity/officetype";
import {Injectable} from "@angular/core";
import {HttpClient} from "@angular/common/http";

@Injectable({
  providedIn: 'root'
})

export class Reviewratingservice{

  constructor( private http: HttpClient) {}

  // @ts-ignore
 async getAllList(): Promise<Array<Reviewrating>>{

    const reviewratings = await this.http.get<Array<Reviewrating>>('http://localhost:8080/reviewratings/list').toPromise();
    if (reviewratings == undefined){
      return [];
    }

    return reviewratings;
 }
}
