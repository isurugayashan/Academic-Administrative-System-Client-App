
import {Schclass} from "../entity/schclass";
import {Civilstatus} from "../entity/civilstatus";
import {Qualification} from "../entity/qualifications";
import {Gradetype} from "../entity/gradetype";
import {Grade} from "../entity/grade";
import {Empstatus} from "../entity/empstatus";
import {Officetype} from "../entity/officetype";
import {Injectable} from "@angular/core";
import {HttpClient} from "@angular/common/http";

@Injectable({
  providedIn: 'root'
})

export class Schclassservice{

  constructor( private http: HttpClient) {}

  // @ts-ignore
 async getAllList(): Promise<Array<Schclass>>{

    const schclasses = await this.http.get<Array<Schclass>>('http://localhost:8080/schclasses/list').toPromise();
    if (schclasses == undefined){
      return [];
    }

    return schclasses;
 }
}
